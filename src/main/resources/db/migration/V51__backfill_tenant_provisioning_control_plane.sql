-- Backfill control-plane tenant provisioning objects for environments that already
-- advanced past V28-V32 while production out-of-order migration was disabled.
--
-- This migration is intentionally idempotent and forward-only:
-- - recreates missing tenant lifecycle columns/constraints
-- - recreates provisioning job/audit tables used by the worker
-- - reapplies the async pending-tenant schema constraint
...
-- Legacy environments may legitimately contain multiple tenants temporarily
-- pointing at the shared `public` schema. Keep uniqueness for real tenant
-- schemas, but do not fail startup on duplicated legacy `public` rows.
