...
class B2BUnitCollectionCashboxAliasControllerTenantIntegrationTest {

    private static final ObjectMapper OBJECT_MAPPER = new ObjectMapper();

    @Test
    void cashCollectionEndpointShouldAcceptCashboxIdAlias() throws Exception {
        ...
        response.setTransactionDate(LocalDate.of(2026, 4, 3));
        response.setTransactionType(B2BUnitTransaction.TransactionType.CASH_COLLECTION);
        response.setAmount(new BigDecimal("90.00"));
        response.setCreditAmount(new BigDecimal("90.00"));
        response.setCashAccountId(77L);
        response.setCreatedAt(LocalDateTime.of(2026, 4, 3, 12, 0));
        ...
        String requestBody = OBJECT_MAPPER.writeValueAsString(new CashAliasPayload(
                "2026-04-03",
                90.00,
                77L
        ));
        ...
        mockMvc.perform(post("/b2b-units/5/collections/cash")
                ...
                .andExpect(jsonPath("$.data.cashAccountId").value(77));
    ...
    private record CashAliasPayload(String transactionDate,
                                    Double amount,
                                    Long cashboxId) {
    }
}
